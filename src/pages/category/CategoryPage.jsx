...
export const CategoryPage = () => {
    ...
    const onFinish = async (item) => {
        ...
        let url = "categories";
        if (id) {
            url = "categories/" + id; // លទ្ធផល: role/14 (ត្រូវជាមួយ Route: role/{id})
        }
    ...
    <div>
        ...
        <Modal 
            ...
            <Form layout='vertical' onFinish={onFinish} form={formRef}>
                ...
                >
                    {state.list.map((item) => (
                        // បើកំពុង Edit មិនត្រូវឱ្យជ្រើសរើសខ្លួនឯងជា Parent ទេ (ការពារ Recursive)
                        <Select.Option 
                            key={item.id} 
                            value={item.id} 
                            disabled={item.id === formRef.getFieldValue("id")}
                        >
                            {item.name}
                        </Select.Option>
                    ))}
